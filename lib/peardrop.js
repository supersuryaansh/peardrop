import Hyperbeam from 'hyperbeam'
import fs from 'fs'
import tar from 'tar-fs'
import path from 'path'
import base32 from 'hi-base32'
import uuid from 'uuid-v4'
import EventEmitter from 'events'
import ui from 'pear-electron'
import sodium from 'sodium-universal'
import b4a from 'b4a'

class Peardrop extends EventEmitter {
  constructor(code) {
    super()
    this.sender = !code
    this._code = code
    this._downloadDir = null // <--- new

    if (this.sender) {
      this._code = uuid().split('-').pop().toUpperCase()
    }

    // Hash the code to exactly 32 bytes for sodium's crypto_sign_seed_keypair
    const seed = b4a.alloc(32)
    sodium.crypto_generichash(seed, b4a.from(this._code))
    const key = base32.encode(Buffer.from(seed)).replace(/=/g, '').toLowerCase()

    this.beam = new Hyperbeam(key, this.sender)
    this._listeners()
  }

  _listeners() {
    this.beam.on('end', () => {
      this.emit('end', 'transfer complete')
    })

    this.beam.on('error', (err) => {
      this.emit('error', err)
    })

    if (!this.sender) {
      this.beam.on('data', (chunk) => {
        this.emit('data', chunk)
      })
    }
  }

  send(items) {
    let rootFolder
    if (items[0].webkitRelativePath) {
      rootFolder = items[0].webkitRelativePath.split('/')[0]
    }

    const files = []
    for (const f of items) {
      const abs = ui.media.getPathForFile(f)
      if (!abs) continue
      if (rootFolder) {
        files.push({ abs, rel: f.webkitRelativePath })
      } else {
        files.push({ abs, rel: path.basename(abs) })
      }
    }

    const pack = tar.pack('/', {
      entries: files.map((f) => f.abs),
      map: (header) => {
        const file = files.find((f) => f.abs.endsWith(header.name))
        if (file) header.name = file.rel
        return header
      }
    })
    pack.pipe(this.beam)
  }

  startReceive() {
    if (this.sender) return
    this._receive()
  }

  _receive() {
    // Use custom download dir if set, else fallback
    const targetDir =
      this._downloadDir || path.join(process.env.HOME || process.env.USERPROFILE, 'Downloads')

    console.log('checking if target dir exists: ', targetDir)
    console.log(this._downloadDir) //null
    if (!fs.existsSync(targetDir)) fs.mkdirSync(targetDir, { recursive: true })
    const extract = tar.extract(targetDir)
    this.beam.pipe(extract)
  }

  /**
   * Set a custom download location
   * @param {string} dirPath
   */
  setDownloadLocation(dirPath) {
    console.log('gona save at:', dirPath)
    this._downloadDir = dirPath
  }

  get code() {
    return this._code
  }

  async destroy() {
    if (this.beam) {
      if (!this.sender) {
        this.beam.end()
      }
      this.beam.destroy()
    }
    this.beam = null
  }
}

export default Peardrop
